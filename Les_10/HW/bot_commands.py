...
def help_command(update: Update, context: CallbackContext):
    ...
    res_str += "Сложение /sub\n"
    res_str += "Умножение /mul\n"
    res_str += "Деление /div\n"
    res_str += "Степень /degree\n"
    res_str += "Комплексное число следует записывать через пробел Real Imag"
...
def degree_command(update: Update, context: CallbackContext):
    global operation
    operation = 5
    ...
    if operation:
        if len(operands)<2:
            ...
            if " " in msg and msg.split(" ")[0].isdigit() and msg.split(" ")[1].isdigit():
                ...
                update.message.reply_text("Введенные дфнные не являются числом. Повторите ввод")
        if len(operands) == 2:
            match operation:
                case 1:
